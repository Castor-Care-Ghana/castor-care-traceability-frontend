...
import {
  Package, Layers, QrCode, User, TrendingUp,
...
const UserDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const [farmersRes, batchesRes, packagesRes, scansRes] = await Promise.all([
        apiGetFarmers(),
        apiGetBatches(),
        apiGetPackages(),
        apiGetScans(),
...
const filteredList = selectedList.filter((item) =>
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Profile Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-300">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3 mt-2 text-sm text-gray-500">
              ...
              <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> Joined {user.createdAt?.slice(0, 10)}</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.id} className="bg-white rounded-xl shadow-sm border border-gray-300 p-6 hover:shadow-md transition">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm border border-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                    {Object.keys(filteredList[0]).slice(0, 5).map((key) => (
                      <th key={key} className="text-left px-3 py-2 border-b border-gray-200 capitalize">
                        {key}
                      </th>
                    ))}
                  </tr>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative border border-gray-300">
